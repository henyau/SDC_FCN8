...
def load_vgg(sess, vgg_path):
    ...
    w3 = tf.get_default_graph().get_tensor_by_name(vgg_layer3_out_tensor_name)
    w4 = tf.get_default_graph().get_tensor_by_name(vgg_layer4_out_tensor_name)
    w7 = tf.get_default_graph().get_tensor_by_name(vgg_layer7_out_tensor_name)
    
    return w1, keep_prob, w3, w4, w7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # 1x1 convolution replaces fully convolutional layer
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    intersection = tf.reduce_sum( logits*correct_label)
    dice = (2.*intersection+1)/(tf.reduce_sum(logits)+tf.reduce_sum(correct_label)+1)
    dice = 1-dice#-tf.reduce_mean(dice)

    train_op = tf.train.AdamOptimizer(learning_rate= learning_rate).minimize(cross_entropy_loss +dice)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # Train NN using train_nn function
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, 
                 input_image,correct_label, keep_prob, learning_rate)

        # Save inference data using delper.save_inference_samples
